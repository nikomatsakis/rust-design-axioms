...
* **Rust users want features to be learnable independently and usable everywhere they make sense**, so we design Rust features to be **composable and orthogonal**. We are looking for features that tackle independent problems and work with one another, giving rise to N×M possibilities. A feature that fits and works with everything else is easier to describe, learn, and use, without an added list of exceptions or unusual interactions. ![WIP][]
